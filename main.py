...
def consumir_mensagens():
    ...
    result = channel.queue_declare(queue='auditor', arguments=args)
    channel.exchange_declare(exchange='dead_letter_exchange', exchange_type='fanout')
    channel.queue_declare(queue='auditor_dlx')
    channel.queue_bind(exchange='dead_letter_exchange', queue='auditor_dlx')
